...
func TestSystemResolversParsing(t *testing.T) {
	dir := t.TempDir()
	path := dir + "/resolv.conf"
	if err := os.WriteFile(path, []byte(`# Generated by resolvconf
...
`), 0o644); err != nil {
	...
	// Point the reader at the fixture.
	orig := resolvConfForTest
	resolvConfForTest = path
	defer func() { resolvConfForTest = orig }()

	got := SystemResolvers()
	want := []string{"127.0.0.1", "1.1.1.1"}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("SystemResolvers() = %v, want %v (comments, junk and bare keywords skipped)", got, want)
	}
}

func TestSystemResolversMissingFile(t *testing.T) {
	orig := resolvConfForTest
	resolvConfForTest = t.TempDir() + "/nope"
	defer func() { resolvConfForTest = orig }()

	if got := SystemResolvers(); got != nil {
		t.Errorf("SystemResolvers() = %v, want nil for a missing file", got)
	}
}
